...
class Mlx:
    ...
    def __init__(self):
        ...
        candidates = [
            os.path.expanduser(f"~/.local/lib/python{py_ver}/site-packages/mlx/libmlx.so"),
            os.path.expanduser("~/.local/lib/python3.10/site-packages/mlx/libmlx.so"),
            "/usr/local/lib/libmlx.so",
            "/usr/lib/libmlx.so",
            os.path.join(module_dir, "libmlx.so"),
        ]
        self.so_file = None
        for path in candidates:
            if os.path.isfile(path):
                self.so_file = path
                break
        if self.so_file is None:
            print(
                "Error: libmlx.so not found. Install MiniLibX first:\n"
                "  make mlx-install",
                file=sys.stderr,
            )
            sys.exit(1)
...
# API break, returns tuple
    def mlx_get_data_addr(self, img_ptr):
        ...
        data = self.mlx_func.mlx_get_data_addr(img_ptr, byref(
            bits_per_pixel), byref(size_line), byref(theformat))
        data_array = c_char * \
            (self._img_height[str(img_ptr)] * size_line.value)
        data_view = data_array.from_address(addressof(data.contents))
        return (memoryview(data_view).cast('B'), bits_per_pixel.value,
                size_line.value, theformat.value)
...
#  Use C^-\ to kill your program.
    ...
    def mlx_hook(self, win_ptr, x_event, x_mask, callback, param):
        ...
        callback_ref = callback_type(callback)
        self._python_ref_gen[str(win_ptr) + "_f_" +
                             str(x_event)] = callback_ref
...
# API break, returns tuple
    def mlx_xpm_file_to_image(self, mlx_ptr, filename):
        ...
        img = self.mlx_func.mlx_xpm_file_to_image(
            mlx_ptr, filename.encode('utf8'), byref(width), byref(height))
        if img is not None:
